...
- `dist/wasm/rhydb_wasm.js`: ES module loader generated by Emscripten. Import this from browser code.
- `dist/wasm/rhydb_wasm.d.ts`: Autogenerated type declarations.
- `dist/wasm/rhydb_wasm.wasm`: compiled SILO C++ code and linked dependencies.
...
The compatibility wrapper `./wasm/scripts/build-wasm.sh` also calls `make wasm`. The Makefile writes the Emscripten Conan profile to `build/wasm/conanprofile-emscripten`, installs dependencies into `build/wasm`, configures CMake with `emcmake`, builds `rhydb_wasm`, and copies the artifacts to `dist/wasm`.
