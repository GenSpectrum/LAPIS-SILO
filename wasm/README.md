...
Building the WASM targets creates these files relative to the RhyDB repository root:
...
- `wasm/dist/wasm64/rhydb_wasm.{js,d.ts,wasm}`: the 64-bit variant, same file names in a `wasm64/`
  subdirectory. Each loader fetches its own `rhydb_wasm.wasm` relative to itself, so the two
  variants coexist without renaming.
...
To use the 64-bit variant, import the `./wasm64` subpath instead. The API is identical; only import from a memory64-capable runtime:

```js
import createRhydbModule from "@rhydb/rhydb-wasm/wasm64";
...
```sh
make wasm     # 32-bit build -> wasm/dist/
make wasm64   # 64-bit (wasm64) build -> wasm/dist/wasm64/
```

The compatibility wrapper `./wasm/scripts/build-wasm.sh` also calls `make wasm`. The Makefile writes the Emscripten Conan profile to `build/wasm/conanprofile-emscripten`, installs dependencies into `build/wasm`, configures CMake with `emcmake`, builds `rhydb_wasm`, and copies the artifacts to `wasm/dist`. `make wasm64` does the same in `build/wasm64` with `--target=wasm64` and copies into `wasm/dist/wasm64`.
...
This builds both variants first if necessary, then runs `node --test wasm/test/*.test.mts`. The
suite runs every test against the 32-bit and the 64-bit build, so it requires Node 24+ for the
64-bit variant.
