...
# Prepare the datasets consumed by the performance benchmarks under localTestData/performance/. The
# synthetic datasets are generated by generate_test_data (see the *_NDJSON_PATH constants in
# performance/sequence_generator.h); the real-data slice is downloaded by the `benchmark_data` CMake
# target (declared via benchmark_dataset() in performance/CMakeLists.txt, change-detected so it is not
# re-downloaded when unchanged). Run this once before running any performance benchmark.
