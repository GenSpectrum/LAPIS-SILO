...
all: build/Debug/silo build/Debug/silo_test

clean:
	find build -name "*.o" -print0 | xargs -0 rm -f
	rm -f build/Debug/silo build/Debug/silo_test
...
build/Debug/silo: $(shell bin/cplusplus-source-files)
...
build/Debug/integration-tests.log: build/Debug/silo build/Debug/preprocessing.log $(shell bin/test-query-files)
...
runapi: build/Debug/silo build/Debug/preprocessing.log
