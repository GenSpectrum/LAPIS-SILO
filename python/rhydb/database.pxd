...
cdef extern from "rhydb/database.h" namespace "rhydb":
    cdef cppclass Database:
        ...
        Roaring getFilteredBitmap(string table_name, string filter) except +handle_rhydb_exception
        void updateColumn(string table_name, string column_name, string value, string filter_expression) except +handle_rhydb_exception
        void saveDatabaseState(string save_directory) except +
        string executeQueryAsArrowIpc(string query_string) except +handle_rhydb_exception
