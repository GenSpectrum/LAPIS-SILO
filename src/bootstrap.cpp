...
unsigned gen_start_N() {
   ...
   std::vector<std::pair<unsigned, unsigned>>
      breaks =  /// histogram, first is count in bucket, second is bucket upper limit
      {
         {2256125, 25},     {5840168, 45},     {12678885, 55},    {12796712, 70},
         {12948586, 80},    {13343882, 150},   {14067025, 400},   {14243091, 1000},
         {14271355, 10000}, {14271938, 21000}, {14480991, 22000}, {14495618, 29903},
      };
   if (r < 328928) {  /// # == 0
      return 0;
   }
   ...
   for (auto br : breaks) {
      if (r < br.first) {  /// # < br -> interpolate
         return (double)(r - last_br.first) / (double)(br.first - last_br.first) *
                (br.second - last_br.second);
      }
   }
...
unsigned gen_end_N() {
   ...
   std::vector<std::pair<unsigned, unsigned>>
      breaks =  /// histogram, first is count in bucket, second is bucket upper limit
      {
         {1823567, 25},     {2898370, 45},     {3921896, 55},     {8623606, 70},
         {11335333, 80},    {12994598, 150},   {14180836, 400},   {14256142, 1000},
         {14492195, 10000}, {14493096, 21000}, {14495440, 22000}, {14495618, 29903},
      };
   if (r < 1823567) {  /// # == 0
      return 0;
   }
...
unsigned gen_N_len() {
   ...
   std::vector<std::pair<unsigned, unsigned>>
      breaks =  /// histogram, first is count in bucket, second is bucket upper limit
      {
         {43610073, 25},    {50662078, 45},    {52933729, 55},    {55229971, 70},
         {56557439, 80},    {62160715, 150},   {75918742, 400},   {77644352, 1000},
         {77869380, 10000}, {77869414, 21000}, {77869415, 22000}, {77869427, 29903},
      };
   if (r < 15439497) {  /// # == 1
      return 1;
   }
...
char sample_pos(std::vector<uint32_t>& dist) {
   ...
   std::cerr << "Shouldn't happen, passed last break 4 " << std::to_string(r) << " "
             << std::to_string(dist.back()) << std::endl;
   return 'A';
}
...
int bootstrap(const silo::Database& db, std::string& out_dir, unsigned seed, unsigned factor) {
   ...
   tbb::parallel_for(
      tbb::blocked_range<unsigned>(0, pango_count),
      [&](tbb::blocked_range<unsigned> local) {
         for (auto pango_id = local.begin(); pango_id < local.end(); pango_id++) {
            ...
            unsigned count = 0;
            for (auto& f : part_filters)
               count += f.getAsConst()->cardinality();
            auto dist = silo::execute_all_dist(db, part_filters);
            char genome[silo::genomeLength];  // Buffer
            for (unsigned rep = 0; rep < factor; rep++) {
               std::string file_name = out_dir + std::to_string(pango_id) + "_" +
                                       std::to_string(rep) + "_" + db.dict->get_pango(pango_id) +
                                       ".fasta";
               /// Generate output file
               {
                  ...
                  for (unsigned i = 0; i < count; i++) {
                     out_file << "EPI_ISL_"
                              << std::to_string(i * epi_factor + rep * pango_count + pango_id)
                              << '\n';
