...
std::optional<common::LineageTreeAndIdMap> Initializer::findLineageTreeForName(
   ...
   std::vector<std::string> possible_suffixes = {
      "",
      ".yaml",
      "_lineage_definition.yaml",
      "_lineage_definitions.yaml",
      "_definition.yaml",
      "_definitions.yaml",
      "_lineage.yaml",
      "LineageDefinition.yaml",
      "LineageDefinitions.yaml",
      "Definition.yaml",
      "Definitions.yaml",
      "Lineage.yaml"
   };
   for (const auto& prefix : possible_prefixes) {
      for (const auto& suffix : possible_suffixes) {
         auto candidate_name = fmt::format("{}{}{}", prefix, lineage_tree_name, suffix);
         for (const auto& [path, lineage_tree] : lineage_trees) {
            if (path.filename() == candidate_name) {
               return lineage_tree;
            }
         }
      }
