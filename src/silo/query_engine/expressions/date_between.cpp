...
std::unique_ptr<filter::operators::Operator> DateBetween::compile(const storage::Table& table
) const {
   CHECK_SILO_QUERY(
   ...
   if (date_column.isSorted()) {
      return std::make_unique<filter::operators::RangeSelection>(
...
std::vector<silo::query_engine::filter::operators::RangeSelection::Range> DateBetween::
   ...
   std::vector<filter::operators::RangeSelection::Range> ranges;
