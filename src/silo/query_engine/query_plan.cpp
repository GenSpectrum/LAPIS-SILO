...
arrow::Result<QueryPlan> QueryPlan::makeQueryPlan(
   ...
   SPDLOG_ERROR(
      "future address: {} in QueryPlan::makeQueryPlan", static_cast<void*>(produce_guard.get())
   );
   return makeQueryPlan(arrow_plan, root, request_id, std::move(produce_guard));
}
...
arrow::Status QueryPlan::executeAndWriteImpl(
   ...
   SPDLOG_ERROR("future address: {}", static_cast<void*>(produce_guard.get()));
