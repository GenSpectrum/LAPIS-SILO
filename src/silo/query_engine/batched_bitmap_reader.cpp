...
std::optional<roaring::Roaring> BatchedBitmapReader::nextBatch() {
   if (filter->isEmpty() || num_rows_produced >= cardinality) {
      return std::nullopt;
   }
   ...
   bool start_in_bitmap = filter->select(num_rows_produced, &start_of_next_batch);
   ...
   bool end_selected = filter->select(proposed_end_rank, &end_of_next_batch);

   if (!end_selected) {
      // Fill batch with remainders. This is not empty because `num_rows_produced < cardinality`
      filter->select(cardinality - 1, &end_of_next_batch);
