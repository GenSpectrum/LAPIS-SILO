...
template <typename SymbolType>
arrow::Schema InsertionAggregation<SymbolType>::getOutputSchema(const silo::schema::TableSchema &table_schema) const {
   std::vector<std::shared_ptr<arrow::Field>> fields;
   fields.push_back(std::make_shared<arrow::Field>(std::string(POSITION_FIELD_NAME), arrow::utf8()));
   fields.push_back(std::make_shared<arrow::Field>(std::string(INSERTED_SYMBOLS_FIELD_NAME), arrow::utf8()));
