...
// TODO.TAE remove shared_ptr
std::shared_ptr<ActionQuery> ActionQuery::parseQuery(const std::string& query_string) {
   try {
      ...
      return std::make_shared<ActionQuery>(std::move(filter), std::move(action));
