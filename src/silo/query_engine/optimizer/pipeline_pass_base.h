...
class PipelinePassBase {
  ...
  public:
   ...
   // Default propagation for the UnionAll operator. Both branches are walked by the same
   // pass instance, so this default is only correct for stateless passes.
   // A stateful pass MUST override this.
   // NOLINTNEXTLINE(misc-no-recursion)
   ...
   // Default propagation for the two-child Join operator. Both branches are walked
   // by the same pass instance, so this default is only correct for stateless passes.
   // A stateful pass MUST override this.
   // NOLINTNEXTLINE(misc-no-recursion)
