...
[[noreturn]] std::unique_ptr<operators::Operator> compileNonBooleanLiteral() {
   SILO_UNIMPLEMENTED();
}
}  // namespace
...
std::unique_ptr<operators::Operator> Int64Literal::compile(const storage::Table& /*table*/) const {
...
std::unique_ptr<operators::Operator> FloatLiteral::compile(const storage::Table& /*table*/) const {
   compileNonBooleanLiteral();
}
...
std::unique_ptr<operators::Operator> StringLiteral::compile(const storage::Table& /*table*/) const {
   compileNonBooleanLiteral();
}
...
std::unique_ptr<operators::Operator> BoolLiteral::compile(const storage::Table& table) const {
   if (value) {
