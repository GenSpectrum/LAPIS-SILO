...
operators::QueryNodePtr bindBaseAction(
   ...
   } else if (dynamic_cast<actions::FastaAligned*>(action_query.action.get()) != nullptr) {
      auto specialized_action = dynamic_cast<actions::FastaAligned*>(action_query.action.get());
      return bindAction(
         specialized_action, std::move(action_query.filter), action_query.table_name, tables
      );
   } else if (dynamic_cast<actions::Fasta*>(action_query.action.get()) != nullptr) {
      auto specialized_action = dynamic_cast<actions::Fasta*>(action_query.action.get());
      return bindAction(
         specialized_action, std::move(action_query.filter), action_query.table_name, tables
      );
   } else if (dynamic_cast<actions::Aggregated*>(action_query.action.get()) != nullptr) {
      auto specialized_action = dynamic_cast<actions::Aggregated*>(action_query.action.get());
      return bindAction(
         specialized_action, std::move(action_query.filter), action_query.table_name, tables
      );
   } else if (dynamic_cast<actions::Mutations<Nucleotide>*>(action_query.action.get()) != nullptr) {
      auto specialized_action =
         dynamic_cast<actions::Mutations<Nucleotide>*>(action_query.action.get());
      return bindAction(
         specialized_action, std::move(action_query.filter), action_query.table_name, tables
      );
   } else if (dynamic_cast<actions::Mutations<AminoAcid>*>(action_query.action.get()) != nullptr) {
      auto specialized_action =
         dynamic_cast<actions::Mutations<AminoAcid>*>(action_query.action.get());
      return bindAction(
         specialized_action, std::move(action_query.filter), action_query.table_name, tables
      );
   } else if (dynamic_cast<actions::InsertionAggregation<Nucleotide>*>(action_query.action.get()) !=
              nullptr) {
      auto specialized_action =
   ...
   } else if (dynamic_cast<actions::InsertionAggregation<AminoAcid>*>(action_query.action.get()) !=
              nullptr) {
      auto specialized_action =
   ...
   } else if (dynamic_cast<actions::PhyloSubtree*>(action_query.action.get()) != nullptr) {
      auto specialized_action = dynamic_cast<actions::PhyloSubtree*>(action_query.action.get());
      return bindAction(
         specialized_action, std::move(action_query.filter), action_query.table_name, tables
      );
   } else if (dynamic_cast<actions::MostRecentCommonAncestor*>(action_query.action.get()) !=
              nullptr) {
      auto specialized_action =
...
operators::QueryNodePtr Binder::bindQuery(
   ...
   auto op = bindBaseAction(std::move(action_query), tables);

   if (!order_by_fields.empty() || randomize) {
      auto field_identifiers = op->getOutputSchema();
      ...
      for (const OrderByField& field : order_by_fields) {
         CHECK_SILO_QUERY(
            std::ranges::contains(field_names, field.name),
      ...
      op = std::make_unique<operators::OrderByNode>(std::move(op), order_by_fields, randomize);
   }

   if (limit.has_value() || offset.has_value()) {
      op = std::make_unique<operators::FetchNode>(std::move(op), limit, offset);
   }
