...
operators::QueryNodePtr optimizeInstance(operators::AggregateNode* node) {
   ...
   if (node->group_by_fields.empty() && node->aggregates.size() == 1 &&
      ...
      if (auto* table_scan_child = dynamic_cast<operators::TableScanNode*>(node->child.get())) {
...
operators::QueryNodePtr Planner::optimize(operators::QueryNodePtr node) {
   if (auto* node_instance = dynamic_cast<operators::AggregateNode*>(node.get())) {
      return optimizeInstance(node_instance);
   }
   if (auto* node_instance = dynamic_cast<operators::OrderByNode*>(node.get())) {
      return optimizeInstance(node_instance);
   }
   if (auto* node_instance = dynamic_cast<operators::FetchNode*>(node.get())) {
      return optimizeInstance(node_instance);
   }
   return node;
}
