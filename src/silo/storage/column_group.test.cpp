...
TEST(ColumnGroup, givenIntegerValueForBoolColumn_returnsColumnInsertError) {
   const auto result =
      setupColumnAndInsertJson<BoolColumn>("bool_col", R"({"bool_col": 42})");
...
TEST(ColumnGroup, givenStringValueForIntColumn_returnsColumnInsertError) {
   const auto result =
      setupColumnAndInsertJson<IntColumn>("int_col", R"({"int_col": "hello"})");
...
TEST(ColumnGroup, givenIntegerValueForDate32Column_returnsColumnInsertError) {
   const auto result =
      setupColumnAndInsertJson<Date32Column>("date_col", R"({"date_col": 42})");
