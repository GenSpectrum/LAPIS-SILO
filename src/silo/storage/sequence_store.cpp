...
void silo::SequenceStorePartition<SymbolType>::optimizeBitmaps() {
   ...
   tbb::parallel_for(tbb::blocked_range<uint32_t>(0, positions.size()), [&](const auto& local) {
      ...
      for (auto position_idx = local.begin(); position_idx != local.end(); ++position_idx) {
         auto& position = positions[position_idx];
         auto symbol_changed = position.deleteMostNumerousBitmap(sequence_count);
...
template <typename SymbolType>
void silo::SequenceStorePartition<SymbolType>::finalizePositions() {
   for (size_t position_idx = 0; position_idx < positions.size(); position_idx++) {
      // TODO(#268) dynamically optimize bitmaps for space and performance
      // -> keep the bitmaps deleted where huge space savings are achieved
      // (should only be some small proportion of all)
      auto& position = positions[position_idx];
      position.undeleteBitmap(sequence_count, position_idx, missing_symbol_bitmaps);
   }
}
