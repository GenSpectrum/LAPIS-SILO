...
void TablePartitionInserter::insert(
   ...
   for (auto sniffed_field : field_order_hint) {
      ...
      error = object[sniffed_field.escaped_key].get(column_value);
      if (error) {
         SPDLOG_WARN(
            "Expected to find the key {} next based on prior sniffing. Falling back to looking "
            "through all keys"
         );
         auto it = std::find_if(object.begin(), object.end(), [&](auto field) {
            if (field.error()) {
               return false;
            }
            std::string_view unescaped_key;
            auto error = field.unescaped_key().get(unescaped_key);
            if (error) {
               return false;
            }
            return unescaped_key == sniffed_field.column_identifier.name;
         });
         if (it == object.end() || (*it).value().get(column_value)) {
            throw silo::append::AppendException(
