...
const QueryTestScenario RANDOMIZE_SEED = {
   .name = "RANDOMIZE_SEED",
   .query = "metadata.details(key, randomize:=1231)",
...
const QueryTestScenario RANDOMIZE_INDEPENDENT_ON_COL_NUMS = {
   ...
   .query = "metadata.details(key, col, randomize:=1231)",
   .expected_query_result = json::parse(
      R"(
[{"col":"A","key":"id5"},
{"col":"A","key":"id1"},
...
const QueryTestScenario DIFFERENT_RANDOMIZE_SEED_DIFFERENT_RESULT = {
   ...
   .query = "metadata.details(key, randomize:=12312)",
   .expected_query_result = json::parse(
      R"([{"key": "id1"},
          {"key": "id3"},
          {"key": "id5"},
...
const QueryTestScenario AGGREGATE_RANDOMIZE = {
   ...
   .query = "metadata.aggregated(key, randomize:=12321)",
   .expected_query_result = json::parse(
      R"([
{"count": 1, "key": "id4"},
{"count": 1, "key": "id5"},
...
const QueryTestScenario ORDER_BY_PRECEDENCE = {
   ...
   .query = "metadata.details(key, col, randomize:=12321).orderBy('col')",
   .expected_query_result = json::parse(
      R"([
{"key": "id5", "col": "A"},
{"key": "id1", "col": "A"},
...
const QueryTestScenario ORDER_BY_AGGREGATE_RANDOMIZE = {
   ...
   .query = "metadata.aggregated(col, randomize:=true).orderBy('count')",
   .expected_query_result = json::parse(
      R"([{"count": 2, "col": "B"},
          {"count": 3, "col": "A"}])"
   )
...
const QueryTestScenario LIMIT_2_RANDOMIZE = {
   ...
   .query =
      "metadata.details(key, col, randomize:=true, limit:=2, offset:=2).orderBy('col', 'key')",
   .expected_query_result = json::parse(
      R"([{"key": "id5", "col": "A"},
          {"key": "id2", "col": "B"}])"
   )
...
const QueryTestScenario LIMIT_3_RANDOMIZE = {
   ...
   .query =
      "metadata.details(key, col, randomize:=true, limit:=3, offset:=2).orderBy('col', 'key')",
   .expected_query_result = json::parse(
      R"([{"key": "id5", "col": "A"},
          {"key": "id2", "col": "B"},
          {"key": "id4", "col": "B"}])"
...
const QueryTestScenario AGGREGATE_LIMIT_RANDOMIZE = {
   ...
   .query = "metadata.aggregated(key, randomize:=12321, limit:=2, offset:=1)",
   .expected_query_result = json::parse(
      R"([{"count": 1, "key": "id5"},
          {"count": 1, "key": "id1"}])"
   )
