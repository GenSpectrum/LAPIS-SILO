...
class MockDatabaseMutex : public silo::api::DatabaseMutex {
  public:
...
class RequestHandlerTestFixture : public ::testing::Test {
  protected:
   ...
   silo::api::test::MockRequest request;
   silo::api::SiloRequestHandlerFactory under_test;

   RequestHandlerTestFixture()
       : database_mutex(),
         request(silo::api::test::MockRequest(response)),
         under_test(database_mutex, silo::config::RuntimeConfig::withDefaults()) {}

   void processRequest(silo::api::SiloRequestHandlerFactory& handler_factory) {
...
TEST_F(
   ...
   silo::api::DatabaseMutex real_database_mutex;

   auto under_test = silo::api::SiloRequestHandlerFactory(
