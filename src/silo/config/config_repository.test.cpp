...
TEST(ConfigRepository, shouldReturnPrimaryKey) {
   ...
   ASSERT_EQ(ConfigRepository(config_reader_mock).getPrimaryKey("test.yaml"), "testPrimaryKey");
}
...
TEST(ConfigRepository, shouldReturnMetadataByName) {
   ...
   const DatabaseConfig valid_config{
      ...
      {
         {"testPrimaryKey", DatabaseMetadataType::STRING},
         {"testMetadata", DatabaseMetadataType::PANGOLINEAGE},
      },
      "testPrimaryKey",
   };
   ...
   ASSERT_EQ(
      ConfigRepository(config_reader_mock).getMetadata("test.yaml", "testMetadata").name,
      "testMetadata"
   );
   ASSERT_EQ(
      ConfigRepository(config_reader_mock).getMetadata("test.yaml", "testMetadata").type,
      DatabaseMetadataType::PANGOLINEAGE
   );
}
...
TEST(ConfigRepository, shouldThrowIfMetadataDoesNotExist) {
   ...
   ASSERT_THROW(
      ConfigRepository(config_reader_mock).getMetadata("test.yaml", "notExistingMetadata"),
      ConfigException
   );
}