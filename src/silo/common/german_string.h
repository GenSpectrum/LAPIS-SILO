...
class GermanString {
  ...
  private:
   std::array<char, I + sizeof(length_type)> data;
  ...
  public:
   ...
   static GermanString makeNullString() {
      GermanString ret;
      std::memset(ret.data.data(), 0, ret.data.size());
      ret.data[ret.data.size() - 1] = 0x01;
      return ret;
   }

   bool isNull() { return length() == 0 && data[data.size() - 1] == 0x01; }
