...
void Database::printAllData(const std::string& table_name) const {
   ...
   auto node = std::make_unique<query_engine::operators::TableScanNode>(
      table, std::make_unique<True>(), std::move(all_columns)
   );

   auto query = node->toQueryPlan(tables, config::QueryOptions{});
   auto query_plan =
      query_engine::QueryPlan::makeQueryPlan(query->plan, query->top_node, "printAllData");
