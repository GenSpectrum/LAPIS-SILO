...
int silo::benchmark_throughput(const Database& db, std::istream& query_defs, const std::string& query_dir_str) {
   ...
   int64_t microseconds = 0;
   {
      ...
      for (auto& query : all_queries) {
         std::ofstream parse_file(par_query_out_dir_str + query.test_name + ".parse.par");
         std::ofstream result_file(par_query_out_dir_str + query.test_name + ".res.par");
         std::ofstream performance_file(par_query_out_dir_str + query.test_name + ".perf.par");
         auto result = execute_query(db, query.query, parse_file, result_file, performance_file);
      }
   }
   std::cout << "Took " << microseconds << " microseconds for " << all_queries.size() << " queries serial.";

   microseconds = 0;
   {
      BlockTimer timer(microseconds);
      tbb::parallel_for_each(all_queries.begin(), all_queries.end(), [&](auto& query) {
         std::ofstream parse_file(query_dir_str + query.test_name + ".parse.par");
         std::ofstream result_file(par_query_out_dir_str + query.test_name + ".res.par");
         std::ofstream performance_file(par_query_out_dir_str + query.test_name + ".perf.par");
         auto result = execute_query(db, query.query, parse_file, result_file, performance_file);
      });
   }
   std::cout << "Took " << microseconds << " microseconds for " << all_queries.size() << " queries parallel.";
