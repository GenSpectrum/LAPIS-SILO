...
/// For every chunk, the resolved groups for this group-by key key are stored
class KeyGroups {
  public:
   KeyGroups() = default;
...
class SequencePositionGrouper : public KeyGroups {
  ...
  public:
   SequencePositionGrouper(
      ...
      for (const auto& [chunk_id, view] : CopyOnWriteBitmap{&column.null_bitmap}) {
  ...
  public:
   explicit IndexedColumnGrouper(const storage::column::DictionaryEncodedColumn& column) {
      ...
      for (size_t group_index = 0; group_index < values.size(); ++group_index) {
         for (const auto& [chunk_id, view] : CopyOnWriteBitmap{values[group_index].second}) {
...
ScalarGroupData buildScalarGroups(
   ...
   for (size_t chunk_id = 0; chunk_id < table.row_layout.numChunks(); ++chunk_id) {
   ...
   for (auto& [value, bitmap] : bitmap_by_value) {
      ...
      for (const auto& [chunk_id, view] : CopyOnWriteBitmap{&data.value_bitmaps.back()}) {
...
/// single map lookup, and `keyValues` returns the typed value array so the output column keeps
